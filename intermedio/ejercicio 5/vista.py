...
class Panel(): 
    def __init__(self, window):
        ...
        var_db_str = StringVar()
        var_db = ttk.Combobox(self.root, textvariable=var_db_str, state="readonly")
        var_db.grid(column=2, row=3)
        var_db["values"] = ("Sqlite", "Mongo DB")
        var_db.set('Sqlite')
        ...
        self.boton_alta=Button(self.root, text="Alta", command=lambda:self.objeto.alta(self.a_val.get(), self.b_val.get(), self.c_val.get(), self.tree,var_db_str.get()))
